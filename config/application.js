...
, start: function() {
    if (!this.started) {
      ...
      App.DB.startup('localhost', 27017, 'rsvpstats', function(error, dbsession){
          console.log('Initializing Twitter DB Session...');
          var TwitterDB = new TwitterDAO(dbsession);

          console.log('Initializing Twitter Service Provider...' + (TwitterDB != null));
          var TwitterProvider = new TwitterService(TwitterDB);
          TwitterProvider.startStream(TwitterProvider);
      });
...
App.require('app/service/TwitterService').TwitterService
App.require('app/service/TwitterDAO').TwitterDAO
